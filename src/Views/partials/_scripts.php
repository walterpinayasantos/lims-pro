        ...
        <script>
            // Esto permite que los archivos JS externos conozcan la URL base
            window.BASE_URL = "<?= BASE_URL ?>";
        </script>

        <?php
        // Si el controlador envió scripts específicos para esta página, los cargamos aquí
        if (isset($data['page_scripts']) && is_array($data['page_scripts'])) {
            foreach ($data['page_scripts'] as $script) {
                // Añadimos un timestamp para evitar caché agresivo durante el desarrollo (?v=time)
